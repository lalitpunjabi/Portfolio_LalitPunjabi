...
import { ArrowRight, Terminal, FileText, Search, Download } from 'lucide-react';
...
export default function HeroSection({ onOpenResume, onOpenPalette }: HeroSectionProps) {
...
{`{
  ...
  "monitoring": ["Prometheus", "Grafana"]
      ...
      case 'experience':
        output = (
          <div className="text-xs text-text-secondary space-y-1 font-mono">
            <p className="text-text-primary font-bold">💼 Internships Timeline:</p>
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center justify-center pt-32 sm:pt-36 md:pt-40 pb-16 px-4 md:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-accent-primary/10 border border-accent-primary/30 text-accent-primary text-xs font-mono">
              ...
              RHCSA Certified • DevOps Engineer
            </div>

            {/* Main Headline */}
            <div className="space-y-2">
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-text-primary">
                Hi, I'm <span className="bg-gradient-to-r from-accent-primary via-purple-400 to-pink-500 bg-clip-text text-transparent">Lalit Punjabi</span>
              </h1>
              <div className="text-xl sm:text-2xl md:text-3xl font-mono text-text-secondary h-10 flex items-center">
                <span></span>
                <span className="text-accent-primary font-bold ml-2 border-b-2 border-accent-primary">{typewriterText}</span>
              </div>
            </div>

            <p className="text-text-secondary text-sm sm:text-base leading-relaxed max-w-xl">
              Specializing in automated cloud infrastructure, Kubernetes orchestration, CI/CD pipeline security, and zero-downtime microservice deployments on AWS.
            </p>

            {/* CTA Buttons */}
            <div className="flex flex-wrap items-center gap-3">
              <a href="#projects" className="btn btn-primary text-xs sm:text-sm px-5 py-3 flex items-center gap-2">
                View Production Projects <ArrowRight size={16} />
              </a>

              {/* Functional Preview & Download Resume Button */}
              <a
                href={resumeUrl}
                target="_blank"
                rel="noreferrer"
                onClick={handleResumeClick}
                className="btn btn-outline text-xs sm:text-sm px-5 py-3 flex items-center gap-2 text-text-secondary hover:text-accent-primary transition-all"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="btn btn-outline text-xs sm:text-sm px-4 py-3 flex items-center gap-1.5 text-text-tertiary hover:text-accent-primary"
              ...
              {onOpenPalette && (
                <button
                  ...
                  className="p-3 rounded-xl bg-secondary/80 hover:bg-accent-primary/10 text-text-tertiary hover:text-accent-primary border border-border-color hover:border-accent-primary/40 transition-all font-mono text-xs flex items-center gap-1.5"
                  title="Open Command Palette (Ctrl + K)"
                >
                  <Search size={16} /> <kbd className="hidden sm:inline text-[10px] bg-black/40 px-1.5 py-0.5 rounded border border-border-color">⌘K</kbd>
                </button>
              )}
            </div>
          ...
          {/* RIGHT COLUMN — PROPORTIONAL PHOTO & INTERACTIVE TERMINAL */}
          <div className="lg:col-span-6 flex flex-col gap-6 items-center lg:items-stretch w-full">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 w-full glass-card p-5 rounded-3xl border border-white/10 bg-[#0f141d]/70 backdrop-blur-md shadow-2xl">
              ...
              <div className="flex-1 space-y-3 font-mono text-xs text-gray-300">
                ...
                <div className="space-y-1.5 text-[11px] text-gray-400 border-t border-white/5 pt-2">
                  <p><span className="text-accent-primary font-bold">Certification:</span> RHCSA Certified</p>
                  <p><span className="text-accent-primary font-bold">Degree:</span> B.Tech AI & Data Science (CGPA 9.43)</p>
                  <p><span className="text-accent-primary font-bold">Cloud:</span> AWS EC2, EKS, Terraform, Ansible</p>
                  <p><span className="text-accent-primary font-bold">Location:</span> Jaipur, Rajasthan, India</p>
                </div>
              </div>
            ...
            <div className="w-full flex flex-col min-w-0">
              ...
              >
                ...
                <div 
                  ref={terminalBodyRef}
                  className="p-4 font-mono text-xs leading-relaxed overflow-y-auto max-h-[220px] text-gray-300 space-y-3"
                >
